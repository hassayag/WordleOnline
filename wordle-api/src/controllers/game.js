...
export const updateGame = async (req, res) => {
    ...
    try {
        await psql().query('UPDATE game SET state = $1 WHERE uuid = $2', [JSON.stringify(game.state), req.params.uuid]);
    }
    catch (err) {
        throw new Error(err.stack);
    }
...
const _getGame = async (uuid) => {
    ...
    try {
        rawGame = await psql().query('SELECT * FROM game WHERE uuid = $1', [uuid]);
    }
    catch (err) {
        throw new Error(err.stack);
    }
